...
public class TaggingService {
    ...
    public Set<ItemTag> generateTags(String itemName, String description) {
        ...
        if (tags.size() < 3) {
            tags.addAll(suggestTagsAsItemTag(fullText));
        }
     ...
     */
    public Set<ItemTag> suggestTagsAsItemTag(String itemName) {
            return this.suggestTags(itemName).stream().map(tag -> {
                ItemTag itemTag = new ItemTag();
                itemTag.setTag(tag);
    ...
    public Set<String> suggestTags(String itemName) {
        try {
            ...
            if (cached.isPresent()) {
                return cached.get().suggestedTags;
            }
